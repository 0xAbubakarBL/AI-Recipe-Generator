...
const ShoppingList = () => {
    ...
    const handleAddItem = async (e) => {
        ...
        try {
            ...
            // safe access to response item
            const addedItem = response.data?.data?.item;
            if (!addedItem) {
                toast.error('Failed to add item (no item returned)');
                return;
            }

            // prefer category from returned item; guard against undefineds
            const newEntry = { ...addedItem, category: addedItem.category || 'Groceries' };

            const updatedItems = [...items, newEntry].filter(Boolean);
            setItems(updatedItems);
            organizeByCategory(updatedItems);

            setNewItem({ name: '', quantity: 1, unit: 'pcs', category: 'Groceries' });
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col">
                <div className="w-full h-16 bg-white border-b border-gray-200 flex items-center px-6 justify-between">
...
const AddItemModal = ({ onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // get the created item from the response and pass it to parent
            const addedItem = response.data?.data?.item;

            toast.success('Item added to shopping list');

            // pass the actual item so parent doesn't add `undefined`
            onSuccess(addedItem);
            onClose();
        } catch (error) {
            toast.error('Failed to add item');
